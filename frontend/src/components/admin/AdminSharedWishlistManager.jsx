...
import {
  Users, Plus, Trash2, Edit2, X, Check, UserPlus, UserMinus, Gift
} from 'lucide-react';
import { toast } from 'react-toastify';
import {
  getSharedWishlistsAdmin,
...
const AdminSharedWishlistManager = ({ familyMembers }) => {
  ...
  const getVisibleHouseholds = (wishlist) => {
    ...
    wishlist.owners.forEach(owner => {
      const ownerData = members.find(m => m.id === owner.id);
  ...
  const loadWishlists = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await getSharedWishlistsAdmin();
  ...
  const handleDeleteWishlist = async (wishlistId) => {
    ...
    try {
      ...
      toast.error('Failed to delete shared wishlist');
    }
  };
  ...
  const handleAddOwner = async (wishlistId) => {
    ...
    const selectedMember = members.find(m => m.id === parseInt(selectedOwnerId, 10));
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Shared Wishlists</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Manage shared wishlists across all households.
        </p>
      </div>

      <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
        <p className="text-sm text-purple-700 dark:text-purple-300">
          <strong>Shared Wishlists</strong> allow multiple people to manage a wishlist together.
          Choose <strong>Normal</strong> to keep purchases secret from owners, or <strong>No Secrets </strong>
          so all co-owners can see and coordinate purchases.
        </p>
      </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setNewWishlistType(e.target.value)}
                className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600
                  bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              >
                ...
                <div className="space-y-3">
                  ...
                  <div className="pt-2 border-t border-gray-200 dark:border-gray-600">
                    ...
                    {(() => {
                      ...
                      const availableMembers = members.filter(m => !currentOwnerIds.includes(m.id));
                ...
                <>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-2 text-sm text-gray-500 dark:text-gray-400 flex-wrap">
                        ...
                        {wishlist.occasion_type === 'birthday' && (
                          <span className="px-2 py-0.5 bg-pink-100 dark:bg-pink-900/50 text-pink-700 dark:text-pink-300 text-xs rounded-full">
                            Birthday
                          </span>
                        )}
                  ...
                  <div className="mt-2 pt-2 border-t border-gray-200 dark:border-gray-700 space-y-1">
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      ...
                      <span className="ml-1">
                        {formatList(wishlist.owners.slice(0, 3).map(owner => owner.name))}
                    ...
                    {(() => {
                      ...
                      if (visibleHouseholds.length > 0) {
                        return (
                          <div className="text-xs text-gray-500 dark:text-gray-400">
                            <span className="font-medium">Visible in households:</span>
                            <span className="ml-1">
...
export default AdminSharedWishlistManager;
