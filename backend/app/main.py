...
def get_all_households_for_user(
    db: Session = Depends(get_db),
    current_user_id: int = Depends(get_current_user_id_from_header)
):
    """Get all households (for any user)"""
    if current_user_id is None:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="User context required")
...
@app.post("/api/households", response_model=schemas.Household)
def create_household_as_user(
    household: schemas.HouseholdCreate,
    db: Session = Depends(get_db),
    current_user_id: int = Depends(get_current_user_id_from_header)
):
    """Create a new household (any user)"""
    if current_user_id is None:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="User context required")
    ...
    try:
        ...
        # Add the creating user to the household
        db.execute(models.user_household_association.insert().values(
...
@app.post("/api/households/{household_id}/join", response_model=schemas.Household)
def join_household(
    household_id: int,
    db: Session = Depends(get_db),
    current_user_id: int = Depends(get_current_user_id_from_header)
):
    """Join a household (any user)"""
    if current_user_id is None:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="User context required")
...
@app.post("/api/households/{household_id}/leave", response_model=schemas.Household)
def leave_household(
    household_id: int,
    db: Session = Depends(get_db),
    current_user_id: int = Depends(get_current_user_id_from_header)
):
    """Leave a household (any user)"""
    if current_user_id is None:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="User context required")
    ...
    try:
        ...
        if result.rowcount == 0:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="You are not in this household")
